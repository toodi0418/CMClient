...
| systemd / launchd qualification | A controlled service-manager fixture starts the same Agent-owned composition | Standard native launch does not require a system service; installers never accept or manufacture runtime credentials | `scripts/cmclient-systemd.sh`, `scripts/cmclient-launchd.sh` and their tests |
| Windows Service qualification | Transitional SCM Service Host exercises bounded Agent shutdown in controlled packaging evidence | The normal Windows product uses current-user resident Agent; Service Host does not own Gateway domain logic | `apps/service-host`, `scripts/cmclient-windows-service.ps1`, packaging tests |
...
| APRS-IS TX/RX monitoring | Gateway periodically flushes the durable outbox and maintains a verified receive-only observer using `<callsignBase>-C<uppercase hex abs(SSID)>` and fixed Taiwan-prefix filter `p/BM/BN/BO/BP/BQ/BU/BV/BW/BX`; the global `t/p` position feed is intentionally excluded. Mapping changes hot-swap only the local exact matcher; provision, observer, or filter changes fence TX and reconnect RX. APRS-IS positive clauses are additive OR filters, so packet types from the listed Taiwan prefixes and port `14580` default messages may arrive, but unrelated packets are neither persisted nor published and only an exact local mapping/outbox/station tuple can confirm delivery. | `GET /api/v1/aprs`, Agent `GatewayProjection::Aprs`, Web, Desktop, and CLI consume one runtime state. Runtime, monitor, and verified-wire regressions are in `apps/gateway/src/aprs-runtime.test.ts`, `aprs-monitor.test.ts`, and `aprs-wire-integration.test.ts`; Web evidence is in `apps/web/src/stores/aprs.ts`, `stores/aprs.test.ts`, and `views/AprsView.vue`. |
| CallMesh verification and mappings | Isolated Gateway client implements the Legacy heartbeat/mappings POST contract, validates normalized mapping/provision snapshots, and persists a monotonic server high-water plus immutable per-hash mapping fingerprints. Exact unchanged responses renew the local provision lease even when the hosted server repeats or rolls back its timestamp; changed content still fails closed. | Web provides mapping and privacy-safe status including provision state; Desktop combines the bounded status with APRS. Agent reads the key only from `~/.cmclient/secrets.json` and transfers it to Gateway over the private bootstrap frame, never argv or environment. |
