...
async function startMonitor(argv) {
  ...
  const hostInput =
    process.env.MESHTASTIC_HOST ||
    (argv.host ? String(argv.host).trim() : '');
  ...
  const tcpPort = Number.isFinite(argv.port)
    ? argv.port
    : process.env.MESHTASTIC_PORT
      ? Number(process.env.MESHTASTIC_PORT)
      : 4403;
