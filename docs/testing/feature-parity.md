...
| CallMesh verification and mappings | Isolated Gateway client validates mapping versions/effective times and fails closed on schema/auth/conflict errors | Web provides the complete mapping/status view; Desktop combines the bounded status with APRS. Agent carries only the configured HTTPS URL and injects the key from OS credential storage. |
