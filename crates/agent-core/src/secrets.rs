...
impl AgentSecretStore {
    ...
    /// Selects the service-safe store only when the packaged systemd unit opts in.
    pub fn runtime(data_dir: &Path) -> Result<Self, SecretStoreError> {
        #[cfg(target_os = "linux")]
        {
            let mode = std::env::var_os(SYSTEMD_SECRET_STORE_ENVIRONMENT);
            let credentials_directory = std::env::var_os(SYSTEMD_CREDENTIALS_DIRECTORY);
            return Self::from_runtime_environment(
                data_dir,
                mode.as_deref(),
...
mod tests {
    use super::{
        AgentSecretStore, SecretBackend, SecretKind, SecretStoreError, ServiceSecretBackend,
        VAULT_MAGIC,
    };
