...
export class CmCloudDirectAprsIgateRuntime {
  ...
  async configure(capability?: CmCloudDirectAprsCapability): Promise<void> {
    ...
    this.running = false;
    this.capabilityState = capability ? "granted" : "not_granted";
    this.profileState = capability ? "invalid" : "missing";
    ...
    try {
      ...
      this.lastErrorCode = "CMCLOUD_DIRECT_APRS_BEACON_PROVISION_INVALID";
      return;
    }
    if (!normalized?.provision) {
      this.profileState = "missing";
      return;
    }
    try {
      const provision = normalized.provision;
      this.family = new AprsIgateFamily({
        provision,
      ...
      this.provisionFingerprint = provisionFingerprint(
        normalized.callsign,
        provision,
      );
      this.running = true;
  ...
  async stop(): Promise<void> {
    ...
    this.provisionFingerprint = undefined;
    this.profileState = "missing";
    this.capabilityState = "not_granted";
    this.beaconActive = false;
