...
此圖用來幫助您一眼看懂 Mesh 網路的骨幹與訊號品質，結構如下：

### 11.1 佈局邏輯（Layout）

當您選擇 **Pyramid (Hubs Top)** 模式時：

- **頂端 (Apex)**：自動找出連接數最多的節點（Hub / 核心中繼），放在金字塔頂端。
- **層級 (Layers)**：其他節點依照與核心節點的「跳數距離」由上往下排列。
- **意義**：越上層通常越重要（中繼站 / Gateway），越下層多為終端用戶。
...
此圖只會顯示**最近 3 小時內**（預設）有發生過 Traceroute 互動的節點，反映「即時網路拓撲」，而非歷史累積的靜態地圖。
